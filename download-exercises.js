...
function parseExercisePage(html, slug) {
    ...
    // Extract SVG URLs (animation versions with &a=1)
    const murlMatch = html.match(/data-murl="([^"]+)"/);
    const furlMatch = html.match(/data-furl="([^"]+)"/);
    if (murlMatch) result.images.male = murlMatch[1];
    if (furlMatch) result.images.female = furlMatch[1];
